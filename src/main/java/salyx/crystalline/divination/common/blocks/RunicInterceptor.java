...
public class RunicInterceptor extends BaseHorizontalBlock{

    public RunicInterceptor(Properties properties) {
        super(properties);
    ...
    public VoxelShape getShape(BlockState state, IBlockReader worldIn, BlockPos pos, ISelectionContext context) {
        return Stream.of(
            ...
            Block.makeCuboidShape(4, 12, 4, 12, 13.5, 12),
            Block.makeCuboidShape(5.5, 1, 5.5, 10.5, 12, 10.5)
            ).reduce((v1, v2) -> {return VoxelShapes.combineAndSimplify(v1, v2, IBooleanFunction.OR);}).get();
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return TileEntityInit.RUNIC_INTERCEPTOR_TILE_TYPE.get().create();
    }
    ...
    public void onReplaced(BlockState state, World worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
        if(!state.isIn(newState.getBlock())) {
            ...
            if(tileEntity instanceof RunicInterceptorTile) {
                InventoryHelper.dropInventoryItems(worldIn, pos, (RunicInterceptorTile)tileEntity);
            }
        }
